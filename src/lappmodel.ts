...
import { loadLive2dWsmAsync, WsmInstance } from './3daf';
...
export class LAppModel extends CubismUserModel {
  private _WsmInstance: WsmInstance;
   ...
   * @param wsmFile
   */
  public loadAssets(dir: string, wsmFile: string): void {
    this._modelHomeDir = dir;

    fetch(dir + wsmFile)
      .then((res) => res.arrayBuffer())
      .then(async (buf) => {
        this._WsmInstance = await loadLive2dWsmAsync(buf);
        const { data: arrayBuffer } = this._WsmInstance.findOne((name) =>
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    if (this._modelSetting.getModelFileName() != '') {
      ...
      const wsmId = this._modelHomeDir.match(/\d+/)[0];

      fetch(`${this._modelHomeDir}${wsmId + '.model.json'}`)
        .then((res) => res.json())
        .then((buf) => {
          // this.loadModel(buf);
          this.setModel(buf);
          ...
          console.log(this.getModel().getModel());
      ...
      if (this._modelSetting.getExpressionCount() > 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const { data: arrayBuffer } =
            this._WsmInstance.read(expressionFileName);
    ...
    const loadCubismPhysics = (): void => {
      if (this._modelSetting.getPhysicsFileName() != '') {
        ...
        const { data: arrayBuffer } = this._WsmInstance.read(physicsFileName);
    ...
    const loadCubismPose = (): void => {
      if (this._modelSetting.getPoseFileName() != '') {
        ...
        const { data: arrayBuffer } = this._WsmInstance.read(poseFileName);
    ...
    const loadUserData = (): void => {
      if (this._modelSetting.getUserDataFile() != '') {
        ...
        const { data: arrayBuffer } = this._WsmInstance.read(userDataFile);
  ...
  private setupTextures(): void {
    ...
    if (this._state == LoadStep.LoadTexture) {
      ...
      for (
        ...
        // wsm
        const { data } = this._WsmInstance.read(texturePath);
        texturePath = URL.createObjectURL(
